...
def image2masklist(frame, model, model_BM,Ovary_threshold,params,img_transform,framelist,video_path,frame_patch_list,i_f):
    ...
    for j in range(len(contours)):
        ...
        if rect_out[0] != 0:
            ...
            out_path = os.path.join(r'C:\Users\Administrator\Desktop\app_test\cut',
...
def pred_video(model,model_BM,device,params,video_path):
    Ovary_threshold=0.1
    print('模型变为测试模式')
    model.eval()
    model_BM.eval()
    ...
    while cap.isOpened():
        ...
        if ret:
            ...
            refine_res, p0_patch_list, p1_patch_list, p2_patch_list=image2masklist(frame, model, model_BM,Ovary_threshold,params,img_transform,framelist,video_path,frame_patch_list,i_f)
            ...
            cv2.imencode('.png', IMG)[1].tofile(
                os.path.join(r"C:\Users\Administrator\Desktop\app_test\frame-for-video-generate",
            ...
            break
        #p0_patch_list, p1_patch_list, p2_patch_list
        print('>>>>>>>>>>>>>>>>>>>>>>>>>>>>',os.path.basename(video_path),'帧',i_f,'预测为:',pred, '分割iou:',siou, '分割dice:',sdice)#, sacc)
    cap.release()
    ...
    for i_f_in in tqdm(range(i_f)):
        framepath=os.path.join(r"C:\Users\Administrator\Desktop\app_test\frame-for-video-generate",
    ...
    return video_path,predList,p0List,p1List,p2List
...
def main(device):
    ...
    videoframeList, videoidList, frameAnnotationList, framepredList, frameP0, frameP1, frameP2 = [], [], [], [], [], [], []
    smoothedtypeList = []
    frameIoU = []
    for video_path in video_pathList[:]:
        # video_path=r"C:\Users\install-44\卵巢动态数据集文件1.1.0.1_to代\有病灶\BaseDB\V_DATA001-GB01-UNK-20230510\OVAB_D_YN01_US-Ovary202108018_0002\OVAB_D_YN01_US-Ovary202108018_0002.mp4"
        video_path,predList,p0List,p1List,p2List=pred_video(model,model_BM,device,opt,video_path)
    ...
    df['p2'] = frameP2
    # df['iou'] = frameIoU
    df.to_csv(r'C:\Users\Administrator\Desktop\app_test\240418orads_video_frame.csv', encoding='ANSI')
